...
[TypeScript](https://www.typescriptlang.org) is a programming language that builds on top of JavaScript. It adds a bunch of features that make it easier to write code, and it also makes it easier to catch bugs before you ship your code. It was created by the inventor of C# and maintained by Microsoft.

We chose TypeScript because it allows us to use a single, strongly-typed language across the entire application.
...
React is a component-based framework, which means that you can build your UI out of small, reusable components. This makes it easy to build complex UIs, and it also makes it easy to reuse code across different parts of your application.

React is also declarative, which means that you describe what you want your UI to look like, and React will take care of updating the UI whenever the data changes. This makes it easy to build complex UIs, and it also makes it easy to reuse code across different parts of your application.

We have a package called [@carbon/react](https://github.com/barbinbrad/carbon/tree/main/packages/carbon-react/src) that has a bunch of React components that you can use to build your UI. You can also use any other React component library that you want.
...
There are a lot of other component libraries out there, but we chose Chakra UI because it gives us a lot of flexibility while still being easy to use. If you're building user interfaces, we recommend you check out their documentation.
...
[Supabase](https://supabase.io) is a database, authentication, and storage service that's built on top of Postgres. It allows us to provide a simple, unified authentication layer across many applications (even one's that don't exist yet).
...
But unlike a raw SQL query, the data that we get back is strongly-typed and nicely nested. It also has nice error reporting baked in.
